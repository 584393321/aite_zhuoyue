 ...
 */
public class ATTimePickerView extends ATBasePickerView implements View.OnClickListener {

    private ATWheelTime wheelTime; //自定义控件
    private static final String TAG_SUBMIT = "submit";
    private static final String TAG_CANCEL = "cancel";

    public ATTimePickerView(ATPickerOptions pickerOptions) {
    ...
    private void initView(Context context) {
        ...
        if (mPickerOptions.customListener == null) {
            LayoutInflater.from(context).inflate(R.layout.at_pickerview_time, contentContainer);
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new ATWheelTime(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
        if (mPickerOptions.timeSelectChangeListener != null) {
            wheelTime.setSelectChangeCallback(new ATISelectTimeCallback() {
                @Override
                public void onTimeSelectChanged() {
                    try {
                        Date date = ATWheelTime.dateFormat.parse(wheelTime.getTime());
    ...
    public void setLunarCalendar(boolean lunar) {
        try {
            int year, month, day, hours, minute, seconds;
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(ATWheelTime.dateFormat.parse(wheelTime.getTime()));
            year = calendar.get(Calendar.YEAR);
            month = calendar.get(Calendar.MONTH);
            day = calendar.get(Calendar.DAY_OF_MONTH);
            hours = calendar.get(Calendar.HOUR_OF_DAY);
