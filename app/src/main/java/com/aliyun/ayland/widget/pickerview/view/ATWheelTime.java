package com.aliyun.ayland.widget.pickerview.view;

import android.view.View;

import com.aliyun.ayland.widget.contrarywind.listener.ATOnItemSelectedListener;
import com.aliyun.ayland.widget.contrarywind.view.ATWheelView;
import com.aliyun.ayland.widget.pickerview.adapter.ATArrayWheelAdapter;
import com.aliyun.ayland.widget.pickerview.adapter.ATNumericWheelAdapter;
import com.aliyun.ayland.widget.pickerview.listener.ATISelectTimeCallback;
import com.aliyun.ayland.widget.pickerview.utils.ATChinaDate;
import com.aliyun.ayland.widget.pickerview.utils.ATLunarCalendar;
...
public class ATWheelTime {
    public static DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private View view;
    private ATWheelView wv_year;
    private ATWheelView wv_month;
    private ATWheelView wv_day;
    private ATWheelView wv_hours;
    private ATWheelView wv_minutes;
    private ATWheelView wv_seconds;
    private int gravity;
    ...
    private ATWheelView.DividerType dividerType;
    private boolean isLunarCalendar = false;
    private ATISelectTimeCallback mSelectChangeCallback;

    public ATWheelTime(View view, boolean[] type, int gravity, int textSize) {
        super();
        this.view = view;
        this.type = type;
    ...
    public void setPicker(int year, final int month, int day, int h, int m, int s) {
        if (isLunarCalendar) {
            int[] lunar = ATLunarCalendar.solarToLunar(year, month + 1, day);
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        // 年
        wv_year = (ATWheelView) view.findViewById(R.id.year);
        wv_year.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getYears(startYear, endYear)));// 设置"年"的显示数据
        wv_year.setLabel("");// 添加文字
        wv_year.setCurrentItem(year - startYear);// 初始化时显示的数据
        wv_year.setGravity(gravity);

        // 月
        wv_month = (ATWheelView) view.findViewById(R.id.month);
        wv_month.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getMonths(year)));
        wv_month.setLabel("");
        
        int leapMonth = ATChinaDate.leapMonth(year);
        ...
        // 日
        wv_day = (ATWheelView) view.findViewById(R.id.day);
        // 判断大小月及是否闰年,用来确定"日"的数据
        if (ATChinaDate.leapMonth(year) == 0) {
            wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.monthDays(year, month))));
        } else {
            wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.leapDays(year))));
        }
        wv_day.setLabel("");
        wv_day.setCurrentItem(day - 1);
        wv_day.setGravity(gravity);

        wv_hours = (ATWheelView) view.findViewById(R.id.hour);
        wv_hours.setAdapter(new ATNumericWheelAdapter(0, 23));
        //wv_hours.setLabel(context.getString(R.string.pickerview_hours));// 添加文字
        wv_hours.setCurrentItem(h);
        wv_hours.setGravity(gravity);

        wv_minutes = (ATWheelView) view.findViewById(R.id.min);
        wv_minutes.setAdapter(new ATNumericWheelAdapter(0, 59));
        ...
        wv_seconds = (ATWheelView) view.findViewById(R.id.second);
        wv_seconds.setAdapter(new ATNumericWheelAdapter(0, 59));
        ...
        // 添加"年"监听
        wv_year.setOnItemSelectedListener(new ATOnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                int year_num = index + startYear;
                // 判断是不是闰年,来确定月和日的选择
                wv_month.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getMonths(year_num)));
                if (ATChinaDate.leapMonth(year_num) != 0 && wv_month.getCurrentItem() > ATChinaDate.leapMonth(year_num) - 1) {
                ...
                int maxItem = 29;
                if (ATChinaDate.leapMonth(year_num) != 0 && wv_month.getCurrentItem() > ATChinaDate.leapMonth(year_num) - 1) {
                    if (wv_month.getCurrentItem() == ATChinaDate.leapMonth(year_num) + 1) {
                        wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.leapDays(year_num))));
                        maxItem = ATChinaDate.leapDays(year_num);
                    } else {
                        wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.monthDays(year_num, wv_month.getCurrentItem()))));
                        maxItem = ATChinaDate.monthDays(year_num, wv_month.getCurrentItem());
                    }
                } else {
                    wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.monthDays(year_num, wv_month.getCurrentItem() + 1))));
                    maxItem = ATChinaDate.monthDays(year_num, wv_month.getCurrentItem() + 1);
                }
        ...
        // 添加"月"监听
        wv_month.setOnItemSelectedListener(new ATOnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                int month_num = index;
                int year_num = wv_year.getCurrentItem() + startYear;
                int maxItem = 29;
                if (ATChinaDate.leapMonth(year_num) != 0 && month_num > ATChinaDate.leapMonth(year_num) - 1) {
                    if (wv_month.getCurrentItem() == ATChinaDate.leapMonth(year_num) + 1) {
                        wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.leapDays(year_num))));
                        maxItem = ATChinaDate.leapDays(year_num);
                    } else {
                        wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.monthDays(year_num, month_num))));
                        maxItem = ATChinaDate.monthDays(year_num, month_num);
                    }
                } else {
                    wv_day.setAdapter(new ATArrayWheelAdapter(ATChinaDate.getLunarDays(ATChinaDate.monthDays(year_num, month_num + 1))));
                    maxItem = ATChinaDate.monthDays(year_num, month_num + 1);
                }
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        // 年
        wv_year = (ATWheelView) view.findViewById(R.id.year);
        wv_year.setAdapter(new ATNumericWheelAdapter(startYear, endYear));// 设置"年"的显示数据
        ...
        if (startYear == endYear) {//开始年等于终止年
            wv_month.setAdapter(new ATNumericWheelAdapter(startMonth, endMonth));
            wv_month.setCurrentItem(month + 1 - startMonth);
        } else if (year == startYear) {
            //起始日期的月份控制
            wv_month.setAdapter(new ATNumericWheelAdapter(startMonth, 12));
            wv_month.setCurrentItem(month + 1 - startMonth);
        } else if (year == endYear) {
            //终止日期的月份控制
            wv_month.setAdapter(new ATNumericWheelAdapter(1, endMonth));
            wv_month.setCurrentItem(month);
        } else {
            wv_month.setAdapter(new ATNumericWheelAdapter(1, 12));
            wv_month.setCurrentItem(month);
        }
        wv_month.setGravity(gravity);
        // 日
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new ATNumericWheelAdapter(startDay, endDay));
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new ATNumericWheelAdapter(startDay, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new ATNumericWheelAdapter(startDay, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {

                    wv_day.setAdapter(new ATNumericWheelAdapter(startDay, 29));
                } else {

                    wv_day.setAdapter(new ATNumericWheelAdapter(startDay, 28));
                }
            }
            wv_day.setCurrentItem(day - startDay);
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new ATNumericWheelAdapter(1, endDay));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new ATNumericWheelAdapter(1, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new ATNumericWheelAdapter(1, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {

                    wv_day.setAdapter(new ATNumericWheelAdapter(1, 29));
                } else {

                    wv_day.setAdapter(new ATNumericWheelAdapter(1, 28));
                }
            }
            wv_day.setCurrentItem(day - 1);
        }
    ...
    private void setChangedListener(ATWheelView wheelView) {
        if (mSelectChangeCallback != null) {
            wheelView.setOnItemSelectedListener(new ATOnItemSelectedListener() {
                @Override
                public void onItemSelected(int index) {
                    mSelectChangeCallback.onTimeSelectChanged();
                }
...
//        int maxItem;
        if (list_big.contains(String.valueOf(monthNum))) {
            ...
            wv_day.setAdapter(new ATNumericWheelAdapter(startD, endD));
//            maxItem = endD;
...
//                maxItem = endD;
    ...
    private String getLunarTime() {
        ...
        if (ATChinaDate.leapMonth(year) == 0) {
            ...
            if ((wv_month.getCurrentItem() + 1) - ATChinaDate.leapMonth(year) <= 0) {
                month = wv_month.getCurrentItem() + 1;
            } else if ((wv_month.getCurrentItem() + 1) - ATChinaDate.leapMonth(year) == 1) {
        ...
        int[] solar = ATLunarCalendar.lunarToSolar(year, month, day, isLeapMonth);
     ...
     * @param dividerType
     */
    public void setDividerType(ATWheelView.DividerType dividerType) {
    ...
    public void setSelectChangeCallback(ATISelectTimeCallback mSelectChangeCallback) {
