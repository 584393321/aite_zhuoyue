 ...
 */
public final class ATInertiaTimerTask extends TimerTask {

    private float mCurrentVelocityY; //当前滑动速度
    private final float mFirstVelocityY;//手指离开屏幕时的初始速度
    private final ATWheelView mWheelView;

    /**
     * @param wheelView 滚轮对象
     * @param velocityY Y轴滑行速度
     */
    public ATInertiaTimerTask(ATWheelView wheelView, float velocityY) {
        super();
    ...
    public final void run() {
        ...
        if (Math.abs(mCurrentVelocityY) >= 0.0F && Math.abs(mCurrentVelocityY) <= 20F) {
            ...
            mWheelView.getHandler().sendEmptyMessage(ATMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        ...
        //刷新UI
        mWheelView.getHandler().sendEmptyMessage(ATMessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
    }
}
