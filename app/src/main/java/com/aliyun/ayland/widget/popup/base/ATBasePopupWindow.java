 ...
 */
package com.aliyun.ayland.widget.popup.base;
...
/**
 * Created by chenjiawei on 2017/11/8.
 */
public abstract class ATBasePopupWindow implements ATBasePopup, PopupWindow.OnDismissListener, ATPopupController {
    private static final String TAG = "BasePopupWindow";
    private static final int MAX_RETRY_SHOW_TIME = 3;
    //元素定义
    private ATPopupWindowProxy mPopupWindow;
    //popup视图
    ...
    public ATBasePopupWindow(Context context) {
        initView(context, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    }

    public ATBasePopupWindow(Context context, int w, int h) {
    ...
    private void initView(Context context, int w, int h) {
        ...
        //默认占满全屏
        mPopupWindow = new ATPopupWindowProxy(mPopupView, w, h, this);
     ...
     * popupwindow是否需要淡入淡出
     */
    public ATBasePopupWindow setNeedPopupFade(boolean needPopupFadeAnima) {
     ...
     * 设置popup的动画style
     */
    public ATBasePopupWindow setPopupAnimaStyle(int animaStyleRes) {
    ...
    private void tryToShowPopup(View v) {
        try {
            ...
            if (autoShowInputMethod && getInputView() != null) {
                ...
                ATInputMethodUtils.showInputMethod(getInputView(), 150);
            }
            retryCounter = 0;
        } catch (Exception e) {
    ...
    private void retryToShowPopup(final View v) {
        ...
        if (isShowing()) dismissWithOutAnima();
        Context context = getContext();
        if (context instanceof Activity) {
     ...
     */
    public ATBasePopupWindow setAdjustInputMethod(boolean needAdjust) {
     ...
     * @return
     */
    public ATBasePopupWindow setAdjustInputMethod(boolean needAdjust, int flag) {
        if (needAdjust) {
     ...
     */
    public ATBasePopupWindow setAutoShowInputMethod(boolean autoShow) {
     ...
     * 在android M之后失效
     */
    public ATBasePopupWindow setBackPressEnable(final boolean backPressEnable) {
     ...
     */
    public ATBasePopupWindow setPopupWindowFullScreen(boolean needFullScreen) {
    ...
    public ATBasePopupWindow setOnDismissListener(OnDismissListener onDismissListener) {
    ...
    public ATBasePopupWindow setOnBeforeShowCallback(OnBeforeShowCallback mOnBeforeShowCallback) {
        this.mOnBeforeShowCallback = mOnBeforeShowCallback;
        return this;
    }

    public ATBasePopupWindow setShowAnimation(Animation showAnimation) {
        if (mShowAnimation != null && mAnimaView != null) {
            mAnimaView.clearAnimation();
    ...
    public ATBasePopupWindow setShowAnimator(Animator showAnimator) {
    ...
    public ATBasePopupWindow setExitAnimation(Animation exitAnimation) {
    ...
    public ATBasePopupWindow setExitAnimator(Animator exitAnimator) {
     ...
     * @param offsetX
     */
    public ATBasePopupWindow setOffsetX(int offsetX) {
     ...
     * @param offsetY
     */
    public ATBasePopupWindow setOffsetY(int offsetY) {
     ...
     * @param popupGravity
     */
    public ATBasePopupWindow setPopupGravity(int popupGravity) {
    ...
    public ATBasePopupWindow setAutoLocatePopup(boolean autoLocatePopup) {
        showAtDown = true;
     ...
     * @deprecated
     */
    public ATBasePopupWindow setShowAtDown(boolean showAtDown) {
     ...
     */
    public ATBasePopupWindow setDismissWhenTouchOuside(boolean dismissWhenTouchOuside) {
    ...
    protected Animation getTranslateAnimation(int start, int end, int durationMillis) {
        return ATSimpleAnimUtil.getTranslateAnimation(start, end, durationMillis);
    }
    ...
    protected Animation getScaleAnimation(float fromX,
                                          ...
                                          int pivotYType,
                                          float pivotYValue) {
        return ATSimpleAnimUtil.getScaleAnimation(fromX, toX, fromY, toY, pivotXType, pivotXValue, pivotYType, pivotYValue);
    }
    ...
    protected Animation getDefaultScaleAnimation() {
        return ATSimpleAnimUtil.getDefaultScaleAnimation();
    }
    ...
    protected Animation getDefaultAlphaAnimation() {
        return ATSimpleAnimUtil.getDefaultAlphaAnimation();
    }
    ...
    protected AnimatorSet getDefaultSlideFromBottomAnimationSet() {
        return ATSimpleAnimUtil.getDefaultSlideFromBottomAnimationSet(mAnimaView);
    }
