package com.aliyun.ayland.widget.magicindicator.buildins.commonnavigator;
...
import com.aliyun.ayland.widget.magicindicator.ATNavigatorHelper;
import com.aliyun.ayland.widget.magicindicator.ATScrollState;
import com.aliyun.ayland.widget.magicindicator.abs.ATIPagerNavigator;
 ...
 */
public class ATCommonNavigator extends FrameLayout implements ATIPagerNavigator, ATNavigatorHelper.OnNavigatorScrollListener {
    private HorizontalScrollView mScrollView;
    private LinearLayout mTitleContainer;
    private LinearLayout mIndicatorContainer;
    private ATIPagerIndicator mIndicator;

    private ATCommonNavigatorAdapter mAdapter;
    private ATNavigatorHelper mNavigatorHelper;
    ...
    private List<ATPositionData> mPositionDataList = new ArrayList<ATPositionData>();
    ...
    public ATCommonNavigator(Context context) {
        super(context);
        mNavigatorHelper = new ATNavigatorHelper();
    ...
    public ATCommonNavigatorAdapter getAdapter() {
        return mAdapter;
    }

    public void setAdapter(ATCommonNavigatorAdapter adapter) {
        if (mAdapter == adapter) {
            return;
        }
        if (mAdapter != null) {
    ...
    private void init() {
        ...
        View root;
        if (mAdjustMode) {
            root = LayoutInflater.from(getContext()).inflate(R.layout.at_pager_navigator_layout_no_scroll, this);
        } else {
            root = LayoutInflater.from(getContext()).inflate(R.layout.at_pager_navigator_layout, this);
        }
    ...
    private void initTitlesAndIndicator() {
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ATIPagerTitleView v = mAdapter.getTitleView(getContext(), i);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mAdapter != null) {
            ...
            if (mReselectWhenLayout && mNavigatorHelper.getScrollState() == ATScrollState.SCROLL_STATE_IDLE) {
    ...
    private void preparePositionData() {
        ...
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ATPositionData data = new ATPositionData();
            ...
            if (v != null) {
                ...
                if (v instanceof ATIMeasurablePagerTitleView) {
                    ATIMeasurablePagerTitleView view = (ATIMeasurablePagerTitleView) v;
        ...
        if (mAdapter != null) {
            ...
            if (mScrollView != null && mPositionDataList.size() > 0 && position >= 0 && position < mPositionDataList.size()) {
                if (mFollowTouch) {
                    ...
                    ATPositionData current = mPositionDataList.get(currentPosition);
                    ATPositionData next = mPositionDataList.get(nextPosition);
    ...
    public ATIPagerIndicator getPagerIndicator() {
        return mIndicator;
    }
    ...
    public void onEnter(int index, int totalCount, float enterPercent, boolean leftToRight) {
        ...
        if (v instanceof ATIPagerTitleView) {
            ((ATIPagerTitleView) v).onEnter(index, totalCount, enterPercent, leftToRight);
        }
    }
    ...
    public void onLeave(int index, int totalCount, float leavePercent, boolean leftToRight) {
        ...
        View v = mTitleContainer.getChildAt(index);
        if (v instanceof ATIPagerTitleView) {
            ((ATIPagerTitleView) v).onLeave(index, totalCount, leavePercent, leftToRight);
        }
    }
    ...
    public void onSelected(int index, int totalCount) {
        ...
        View v = mTitleContainer.getChildAt(index);
        if (v instanceof ATIPagerTitleView) {
            ((ATIPagerTitleView) v).onSelected(index, totalCount);
        }
        if (!mAdjustMode && !mFollowTouch && mScrollView != null && mPositionDataList.size() > 0) {
            ...
            ATPositionData current = mPositionDataList.get(currentIndex);
    ...
    public void onDeselected(int index, int totalCount) {
        ...
        View v = mTitleContainer.getChildAt(index);
        if (v instanceof ATIPagerTitleView) {
            ((ATIPagerTitleView) v).onDeselected(index, totalCount);
        }
    }

    public ATIPagerTitleView getPagerTitleView(int index) {
        if (mTitleContainer == null) {
            return null;
        }
        return (ATIPagerTitleView) mTitleContainer.getChildAt(index);
    }
