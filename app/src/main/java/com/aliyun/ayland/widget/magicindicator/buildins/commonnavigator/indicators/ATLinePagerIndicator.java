package com.aliyun.ayland.widget.magicindicator.buildins.commonnavigator.indicators;
...
import com.aliyun.ayland.widget.magicindicator.ATFragmentContainerHelper;
import com.aliyun.ayland.widget.magicindicator.buildins.ATArgbEvaluatorHolder;
import com.aliyun.ayland.widget.magicindicator.buildins.ATUIUtil;
 ...
 * eated by hackware on 2016/6/26.
 */
public class ATLinePagerIndicator extends View implements ATIPagerIndicator {
    ...
    private List<ATPositionData> mPositionDataList;
    private List<Integer> mColors;

    private RectF mLineRect = new RectF();

    public ATLinePagerIndicator(Context context) {
        super(context);
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        mPaint.setColor(getResources().getColor(R.color._86523C));
        mLineHeight = ATUIUtil.dip2px(context, 3);
        mLineWidth = ATUIUtil.dip2px(context, 10);
    }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mColors != null && mColors.size() > 0) {
            ...
            int color = ATArgbEvaluatorHolder.eval(positionOffset, currentColor, nextColor);
            mPaint.setColor(color);
        }

        // 计算锚点位置
        ATPositionData current = ATFragmentContainerHelper.getImitativePositionData(mPositionDataList, position);
        ATPositionData next = ATFragmentContainerHelper.getImitativePositionData(mPositionDataList, position + 1);
    ...
    @Override
    public void onPositionDataProvide(List<ATPositionData> dataList) {
