package com.aliyun.ayland.widget.contrarywind.view;
...
import com.aliyun.ayland.widget.contrarywind.interfaces.ATIPickerViewData;
import com.aliyun.ayland.widget.contrarywind.listener.ATLoopViewGestureListener;
import com.aliyun.ayland.widget.contrarywind.listener.ATOnItemSelectedListener;
import com.aliyun.ayland.widget.contrarywind.timer.ATInertiaTimerTask;
import com.aliyun.ayland.widget.contrarywind.timer.ATMessageHandler;
import com.aliyun.ayland.widget.contrarywind.timer.ATSmoothScrollTimerTask;
 ...
 * 3d滚轮控件
 */
public class ATWheelView extends View {
    ...
    private ATOnItemSelectedListener onItemSelectedListener;
    ...
    private ATWheelAdapter adapter;
    ...
    public ATWheelView(Context context) {
        this(context, null);
    }

    public ATWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initLoopView(Context context) {
        ...
        handler = new ATMessageHandler(this);
        gestureDetector = new GestureDetector(context, new ATLoopViewGestureListener(this));
    ...
    public void smoothScroll(ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new ATSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    public final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new ATInertiaTimerTask(this, velocityY), 0, VELOCITY_FLING, TimeUnit.MILLISECONDS);
    }
    ...
    public final void setOnItemSelectedListener(ATOnItemSelectedListener OnItemSelectedListener) {
        this.onItemSelectedListener = OnItemSelectedListener;
    }

    public final void setAdapter(ATWheelAdapter adapter) {
        this.adapter = adapter;
        reMeasure();
        invalidate();
    }

    public final ATWheelAdapter getAdapter() {
        return adapter;
    }
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
    ...
    private String getContentText(Object item) {
        if (item == null) {
            return "";
        } else if (item instanceof ATIPickerViewData) {
            return ((ATIPickerViewData) item).getPickerViewText();
