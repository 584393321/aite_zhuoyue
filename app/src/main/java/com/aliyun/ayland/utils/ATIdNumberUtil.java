...
public class ATIdNumberUtil {
     ...
     * @param idCardNum
     */
    public ATIdNumberUtil(String idCardNum) {
        // super();
    ...
    public int isCorrect() {
        if (this.isEmpty()) {
            this.error = ATIdNumberUtil.IS_EMPTY;
            return this.error;
        }

        if (!this.isLenCorrect()) {
            this.error = ATIdNumberUtil.LEN_ERROR;
            return this.error;
        }

        if (!this.isCharCorrect()) {
            this.error = ATIdNumberUtil.CHAR_ERROR;
            return this.error;
        }

        if (!this.isDateCorrect()) {
            this.error = ATIdNumberUtil.DATE_ERROR;
            return this.error;
        }

        if (this.is18()) {
            if (!this.getCheck().equals(this.getCheckBit())) {
                this.error = ATIdNumberUtil.CHECK_BIT_ERROR;
                return this.error;
            }
        }
