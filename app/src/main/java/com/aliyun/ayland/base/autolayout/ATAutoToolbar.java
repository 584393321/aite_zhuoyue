...
public class ATAutoToolbar extends Toolbar {
    ...
    public ATAutoToolbar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray menuA = context.getTheme().obtainStyledAttributes(attrs, R.styleable.Toolbar,
                defStyleAttr, R.style.ThemeOverlay_AppCompat);
        int menuTextAppearance = menuA.getResourceId(R.styleable.Toolbar_titleTextAppearance,
    ...
    public ATAutoToolbar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ATAutoToolbar(Context context) {
        this(context, null);
    }
    ...
    private void setUpTitleTextSize(String name, int val) {
        try {
            ...
            if (textView != null) {
                int autoTextSize = ATAutoUtils.getPercentHeightSize(val);
    ...
    public static class LayoutParams extends Toolbar.LayoutParams implements ATAutoLayoutHelper.AutoLayoutParams {
        private ATAutoLayoutInfo mDimenLayoutInfo;

        public LayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            this.mDimenLayoutInfo = ATAutoLayoutHelper.getAutoLayoutInfo(c, attrs);
        }
